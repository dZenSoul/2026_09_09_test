...
Все JSON-ответы используют ровно одно верхнеуровневое поле: `response`, `data` или `error`. Исключение — успешное скачивание файла, возвращающее исходные байты. Прикладной `error.code` стабилен и соответствует HTTP-статусу (`400`, `401`, `403`, `404`, `405`, `500`, `501`). Для `405` возвращается `Allow`.
